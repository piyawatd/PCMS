...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('adminproductnew') }}" class="btn btn-primary btn-icon-split btn-sm">
    ...
    <script type="text/javascript">
        ...
        function createtable(){
            oTable = $('#dataTable').DataTable({
                ...
                "pageLength": 25,
                "ajax":{
                    "url": "{{ route('adminproductlist') }}?category="+$('#category').val(),
                    "dataType": "json",
                    "type": "GET",
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function () {
                        $.ajax({
                            type: "PUT",
                            url: "{{ url('admins/product/publish') }}/"+id,
                            dataType:"json",
                            data:{ _token: "{{csrf_token()}}" },
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function () {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ url('admins/product/delete') }}/"+id,
                            dataType:"json",
                            data:{ _token: "{{csrf_token()}}" },
                            success: function(response){
                                if(response.result == true)
