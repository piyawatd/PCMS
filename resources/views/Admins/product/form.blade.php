...
@section('content')
    <?php
    $headertitle = ' - เพิ่ม';
    $linkurl = route('adminproductsave');
    if ($mode == 'edit'){
        $headertitle = ' - แก้ไข '.$product->name_th;
        $linkurl = route('adminproductupdate',['id'=>$product->id]);
    }
    ?>
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">สินค้า{{$headertitle}}</h1>
    </div>
    ...
    <div class="row">
        <div class="col-md-11">
            <form class="needs-validation" id="formdata" novalidate method="post" action="{{$linkurl}}">
                ...
                <a href="{{ route('adminproduct') }}" class="btn btn-danger btn-sm">ยกเลิก</a>
            </form>
        </div>
    </div>
@endsection
                    ...
                    $.ajax({
                        ...
                        success:function (result) {
                            if (result.value == true) {
                                ...
                                $('#validatealert').text('Alias นี้มีสินค้าอื่นใช้แล้ว');
                                pass = false;
                            } else {
